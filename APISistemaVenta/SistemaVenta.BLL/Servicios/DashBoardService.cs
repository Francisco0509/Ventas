...
namespace SistemaVenta.BLL.Servicios
{
    public class DashBoardService : IDashBoardService
    {
        ...
        {
            DateTime? ultimaFecha = tablaVentas.OrderByDescending(x => x.FechaRegistro).Select(v => v.FechaRegistro).First();
        ...
        private async Task<int> TotalProductos()
        {
            ...
            var total = _productoQuery.Count();
        ...
        private async Task<Dictionary<string, int>> VentasUltimaSemana()
        {
            ...
            { 
                ...
                resultado = tablaVenta
                    .GroupBy(x => x.FechaRegistro.Value.Date).OrderBy(g => g.Key)
                    .Select(dv => new { fecha = dv.Key.ToString("dd/MM/yyy"), 
                                        total = dv.Count() }).ToDictionary(keySelector:r => r.fecha, elementSelector: r => r.total);
            }
